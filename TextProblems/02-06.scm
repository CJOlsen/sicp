...
(define zero (lambda (f) (f (lambda (x) x))))
...
;substitution
(define a (add-1 zero))
(define a (lambda (f) (lambda (x) (f ((zero f) x)))))
(define a (lambda (f) (lambda (x) (f ((lambda (f) (f (lambda (x) x))) f) x))))
(define a (lambda (f) (lambda (x) (f.... 
